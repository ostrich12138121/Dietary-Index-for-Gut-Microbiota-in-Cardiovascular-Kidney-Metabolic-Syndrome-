...
def get_output_dir():
    try:
        ...
        base_dir = os.getcwd()
    out_dir = os.path.join(base_dir, "rsf_no_year_metrics_outputs")
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, valid_idx) in enumerate(skf.split(X_raw, event_all), start=1):
        ...
        # 3) Brier score / IBS
        surv_fns = rsf.predict_survival_function(X_valid)
        surv_mat = np.row_stack([fn(EVAL_TIMES) for fn in surv_fns])
    ...
    fold_path = os.path.join(out_dir, f"RSF_{ENDPOINT}_metrics_5fold.csv")
    summary_path = os.path.join(out_dir, f"RSF_{ENDPOINT}_metrics_summary.txt")
